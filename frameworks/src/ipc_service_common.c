...
int32_t IpcServiceGmRegCallback(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    int32_t cbObjIdx = -1;
    LOGI("starting ...");
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_APPID, &appId);
    if (ret != HC_SUCCESS) {
        LOGE("IpcServiceGmRegCallback failed, get app id error.");
        return HC_ERR_IPC_BAD_PARAM;
    }
    ret = GetAndValSizeStructParam(ipcParams, paramNum, PARAM_TYPE_DEV_AUTH_CB, (uint8_t *)&callback,
    ...
    ret = GetAndValSize32Param(ipcParams, paramNum, PARAM_TYPE_CB_OBJECT, &cbObjIdx);
    if (ret != HC_SUCCESS) {
...
int32_t IpcServiceGmRegDataChangeListener(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValSizeStructParam(ipcParams, paramNum, PARAM_TYPE_LISTENER, (uint8_t *)&callback,
...
int32_t IpcServiceGmCreateGroup(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    const char *appId = NULL;
    LOGI("starting ...");
    ret = GetAndValSize32Param(ipcParams, paramNum, PARAM_TYPE_OS_ACCOUNT_ID, &osAccountId);
    if (ret != HC_SUCCESS) {
        LOGE("IpcServiceGmCreateGroup failed, get os account id error.");
        return ret;
    }
    ret = GetAndValSize64Param(ipcParams, paramNum, PARAM_TYPE_REQID, &requestId);
    if (ret != HC_SUCCESS) {
    ...
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_CREATE_PARAMS, &createParams);
    if (ret != HC_SUCCESS) {
...
int32_t IpcServiceGmDelGroup(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_DEL_PARAMS, &delParams);
    if (ret != HC_SUCCESS) {
...
int32_t IpcServiceGmAddMemberToGroup(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_ADD_PARAMS, &addParams);
    if (ret != HC_SUCCESS) {
...
int32_t IpcServiceGmProcessData(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = BindRequestIdWithAppId(data);
    if (ret != HC_SUCCESS) {
        return ret;
    }
    callRet = g_devGroupMgrMethod.processData(requestId, (uint8_t *)data, dataLen);
    ret = IpcEncodeCallReply(outCache, PARAM_TYPE_IPC_RESULT, (const uint8_t *)&callRet, sizeof(int32_t));
    LOGI("process done, call ret %" LOG_PUB "d, ipc ret %" LOG_PUB "d", callRet, ret);
    return ret;
}
...
int32_t IpcServiceGmApplyRegisterInfo(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    LOGI("starting ...");
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_REQ_JSON, &reqJsonStr);
    if (ret != HC_SUCCESS) {
        return ret;
    }
...
int32_t IpcServiceGmCheckAccessToGroup(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_GROUPID, &groupId);
    if (ret != HC_SUCCESS) {
...
int32_t IpcServiceGmGetPkInfoList(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_QUERY_PARAMS, &queryParams);
    if (ret != HC_SUCCESS) {
        return ret;
    }
...
int32_t IpcServiceGmGetJoinedGroups(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValSize32Param(ipcParams, paramNum, PARAM_TYPE_GROUP_TYPE, &groupType);
    if (ret != HC_SUCCESS) {
        return HC_ERR_IPC_BAD_PARAM;
    }
...
int32_t IpcServiceGmGetRelatedGroups(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_UDID, &peerUdid);
    if (ret != HC_SUCCESS) {
...
int32_t IpcServiceAvGetClientSharedKey(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    int32_t ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_PK_WITH_SIG, &peerPkWithSig);
    if (ret != HC_SUCCESS) {
        LOGE("get param error, type %" LOG_PUB "d", PARAM_TYPE_PK_WITH_SIG);
        return HC_ERR_IPC_BAD_PARAM;
    }
    const char *serviceId = NULL;
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_SERVICE_ID, &serviceId);
    if (ret != HC_SUCCESS) {
...
int32_t IpcServiceAvGetServerSharedKey(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValParam(ipcParams, paramNum, PARAM_TYPE_RANDOM, (uint8_t *)&randomVal, &randomLen);
    if (ret != HC_SUCCESS) {
...
int32_t IpcServiceLaStartLightAccountAuth(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValSizeStructParam(ipcParams, paramNum, PARAM_TYPE_DEV_AUTH_CB, (uint8_t *)&laCallBack,
        sizeof(DeviceAuthCallback));
    if (ret != HC_SUCCESS) {
        return ret;
    }
...
int32_t IpcServiceLaProcessLightAccountAuth(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValParam(ipcParams, paramNum, PARAM_TYPE_COMM_DATA, (uint8_t *)&msgVal, &msgLen);
    if (ret != HC_SUCCESS) {
...
int32_t IpcServiceGmIsDeviceInGroup(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_UDID, &udid);
    if (ret != HC_SUCCESS) {
...
int32_t IpcServiceGaProcessData(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    LOGI("starting ...");
    ret = GetAndValSize64Param(ipcParams, paramNum, PARAM_TYPE_REQID, &reqId);
    if (ret != HC_SUCCESS) {
        return ret;
    }
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_COMM_DATA, &data);
    ...
    dataLen = HcStrlen(data) + 1;
    ret = GetAndValSizeStructParam(ipcParams, paramNum, PARAM_TYPE_DEV_AUTH_CB, (uint8_t *)&gaCallback,
        sizeof(DeviceAuthCallback));
    if (ret != HC_SUCCESS) {
        return ret;
    }
    ...
    callRet = g_groupAuthMgrMethod.processData(reqId, (const uint8_t *)data, dataLen, &g_authCbAdt);
    if (callRet != HC_SUCCESS) {
        DelIpcCallBackByReqId(reqId, CB_TYPE_TMP_DEV_AUTH, true);
    }
...
int32_t IpcServiceGaAuthDevice(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_AUTH_PARAMS, &authParams);
    if (ret != HC_SUCCESS) {
        return ret;
    }
    ret = GetAndValSizeStructParam(ipcParams, paramNum, PARAM_TYPE_DEV_AUTH_CB, (uint8_t *)&gaCallback,
...
int32_t IpcServiceGaGetRealInfo(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_PSEUDONYM_ID, &pseudonymId);
    if (ret != HC_SUCCESS) {
        return ret;
    }
...
int32_t IpcServiceGaGetPseudonymId(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_INDEX_KEY, &indexKey);
    if (ret != HC_SUCCESS) {
        return ret;
    }
...
int32_t IpcServiceDaProcessCredential(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    LOGI("starting ...");
    ret = GetAndValSize32Param(ipcParams, paramNum, PARAM_TYPE_OPCODE, &operationCode);
    if (ret != HC_SUCCESS) {
...
int32_t IpcServiceDaProcessData(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    int32_t cbObjIdx = -1;
    LOGI("starting ...");
    ret = GetAndValSize64Param(ipcParams, paramNum, PARAM_TYPE_REQID, &authReqId);
    if (ret != HC_SUCCESS) {
...
int32_t IpcServiceCmAddCredential(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_REQUEST_PARAMS, &requestParams);
    if (ret != HC_SUCCESS) {
...
int32_t IpcServiceCmRegCredChangeListener(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValSizeStructParam(ipcParams, paramNum, PARAM_TYPE_LISTENER, (uint8_t *)&listener,
...
int32_t IpcServiceCmExportCredential(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_CRED_ID, &credId);
    if (ret != HC_SUCCESS) {
...
int32_t IpcServiceCmQueryCredentialByParams(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_QUERY_PARAMS, &requestParams);
    if (ret != HC_SUCCESS) {
...
int32_t IpcServiceCmAgreeCredential(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    ret = GetAndValStringParam(ipcParams, paramNum, PARAM_TYPE_CRED_ID, &selfCredId);
    if (ret != HC_SUCCESS) {
...
int32_t IpcServiceCaProcessCredData(const IpcDataInfo *ipcParams, int32_t paramNum, uintptr_t outCache)
{
    ...
    AddIpcCbObjByReqId(reqId, cbObjIdx, CB_TYPE_CRED_DEV_AUTH);
    InitDeviceAuthCbCtx(&g_authCbAdt, CB_TYPE_CRED_DEV_AUTH);
    callRet = g_credAuthMgrMethod.processCredData(reqId, (const uint8_t *)data, dataLen, &g_authCbAdt);
    if (callRet != HC_SUCCESS) {
        DelIpcCallBackByReqId(reqId, CB_TYPE_CRED_DEV_AUTH, true);
    }
    return IpcEncodeCallReply(outCache, PARAM_TYPE_IPC_RESULT, (const uint8_t *)&callRet, sizeof(int32_t));
