...
static int32_t CreateServerKeyPair()
{
    ...
    const char *requestParams =
        CreateCredentialParam(TEST_AUTH_OS_ACCOUNT_ID, selfUdid, RETURN_FLAG_PUBLIC_KEY, DEFAULT_SERVICE_TYPE, nullptr);
    free(selfUdid);
    char *returnData = nullptr;
...
static int32_t DeleteServerKeyPair()
{
    char *selfUdid = GetSelfUdid();
    const char *requestParams =
        CreateCredentialParam(TEST_AUTH_OS_ACCOUNT_ID, selfUdid, 1, DEFAULT_SERVICE_TYPE, nullptr);
    free(selfUdid);
    char *returnData = nullptr;
...
{
    const char *requestParams = CreateCredentialParam(TEST_AUTH_OS_ACCOUNT_ID, TEST_UDID, -1, importServiceType,
