...
{
    char *returnJsonStr = nullptr;
    int32_t ret = GenerateBindSuccessData(nullptr, NORMAL_STR, &returnJsonStr);
    ASSERT_NE(ret, HC_SUCCESS);
}
...
{
    char *returnJsonStr = nullptr;
    int32_t ret = GenerateBindSuccessData(NORMAL_STR, nullptr, &returnJsonStr);
    ASSERT_NE(ret, HC_SUCCESS);
}

HWTEST_F(GroupOperationTest, GroupOperationTest039, TestSize.Level0)
{
    int32_t ret = GenerateBindSuccessData(NORMAL_STR, NORMAL_STR, nullptr);
    ASSERT_NE(ret, HC_SUCCESS);
}
