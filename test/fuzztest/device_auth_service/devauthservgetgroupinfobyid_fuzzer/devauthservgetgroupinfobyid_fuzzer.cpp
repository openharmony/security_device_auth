...
#include "devauthservgetgroupinfobyid_fuzzer.h"
...
namespace OHOS {
    ...
    bool FuzzDoGetGroupInfoById(const uint8_t* data, size_t size)
    {
        ...
        char *returnGroupInfo = nullptr;
        gmInstance->getGroupInfoById(0, appId.c_str(), g_groupId, &returnGroupInfo);
        if (returnGroupInfo != nullptr) {
            gmInstance->destroyInfo(&returnGroupInfo);
        }
