...
#include "devauthservgetdeviceinfobyid_fuzzer.h"
...
namespace OHOS {
    ...
    bool FuzzDoGetDeviceInfoById(const uint8_t* data, size_t size)
    {
        ...
        char *returnDeviceInfo = nullptr;
        gmInstance->getDeviceInfoById(0, appId.c_str(), localUdid, g_groupId, &returnDeviceInfo);
        if (returnDeviceInfo != nullptr) {
            gmInstance->destroyInfo(&returnDeviceInfo);
        }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::FuzzDoGetDeviceInfoById(data, size);
    return 0;
}
