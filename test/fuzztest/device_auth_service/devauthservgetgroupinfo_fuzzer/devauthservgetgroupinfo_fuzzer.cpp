...
#include "devauthservgetgroupinfo_fuzzer.h"
...
namespace OHOS {
    ...
    static void GenerateReqJson(CJson *json, const char *appId)
    {
        AddStringToJson(json, FIELD_GROUP_ID, g_groupId);
        AddStringToJson(json, FIELD_GROUP_NAME, appId);
        AddStringToJson(json, FIELD_GROUP_OWNER, appId);
    ...
    bool FuzzDoGetGroupInfo(const uint8_t* data, size_t size)
    {
        ...
        GenerateReqJson(reqJson, appId.c_str());
        char *reqJsonStr = PackJsonToString(reqJson);
        FreeJson(reqJson);
        char *returnGroupVec = nullptr;
        uint32_t groupNum = 0;
        gmInstance->getGroupInfo(0, appId.c_str(), reqJsonStr, &returnGroupVec, &groupNum);
