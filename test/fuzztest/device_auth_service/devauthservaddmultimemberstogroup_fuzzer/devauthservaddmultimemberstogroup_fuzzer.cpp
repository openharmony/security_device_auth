...
#include "devauthservaddmultimemberstogroup_fuzzer.h"
...
namespace OHOS {
    ...
    static void GenerateAddMultiMemberParamsJson(CJson *json)
    {
    ...
    bool FuzzDoAddMultiMembersToGroup(const uint8_t* data, size_t size)
    {
        if (data == nullptr) {
            return false;
        }
        ...
        CJson *addMultiMemberJson = CreateJson();
        GenerateAddMultiMemberParamsJson(addMultiMemberJson);
        char *addMultiMembersParams = PackJsonToString(addMultiMemberJson);
        FreeJson(addMultiMemberJson);
        LOGI("add multi members params str:%" LOG_PUB "s", addMultiMembersParams);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::FuzzDoAddMultiMembersToGroup(data, size);
    return 0;
}
