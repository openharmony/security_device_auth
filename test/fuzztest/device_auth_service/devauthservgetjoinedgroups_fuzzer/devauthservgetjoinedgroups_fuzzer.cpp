...
#include "devauthservgetjoinedgroups_fuzzer.h"
...
namespace OHOS {
    ...
    bool FuzzDoGetJoinedGroups(const uint8_t* data, size_t size)
    {
        ...
        uint32_t groupNum = 0;
        gmInstance->getJoinedGroups(0, appId.c_str(), PEER_TO_PEER_GROUP, &returnGroupVec, &groupNum);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::FuzzDoGetJoinedGroups(data, size);
    return 0;
}
