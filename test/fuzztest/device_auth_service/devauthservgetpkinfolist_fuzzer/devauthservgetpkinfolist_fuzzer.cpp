...
#include "devauthservgetpkinfolist_fuzzer.h"
...
namespace OHOS {
    ...
    static void GenerateReqJson(CJson *json)
    {
    ...
    bool FuzzDoGetPkInfoList(const uint8_t* data, size_t size)
    {
        ...
        GenerateReqJson(reqJson);
        char *reqJsonStr = PackJsonToString(reqJson);
        FreeJson(reqJson);
        char *queryResult = nullptr;
        uint32_t resultNum = 0;
        gmInstance->getPkInfoList(0, appId.c_str(), reqJsonStr, &queryResult, &resultNum);
        if (queryResult != nullptr) {
            gmInstance->destroyInfo(&queryResult);
        }
