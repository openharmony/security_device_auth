...
static int32_t ParseIsoAuthClientBeginMsg(IsoAuthParams *params, const CJson *in)
{
    ...
    if (res != HC_SUCCESS) {
        LOGE("ExtractAndVerifyPayload failed for server, res: %" LOG_PUB "d.", res);
    }
    return res;
}
...
static int32_t PackIsoAuthServerGetTokenMsg(const IsoAuthParams *params, CJson *out)
{
    ...
    if (res != HC_SUCCESS) {
        LOGE("AddGetTokenDataToJson failed, res: %" LOG_PUB "d.", res);
        FreeJson(sendToPeer);
        return res;
    }
...
static int32_t IsoAuthServerGetToken(TaskBase *task, const CJson *in, CJson *out, int32_t *status)
{
    ...
    if (res != HC_SUCCESS) {
        LOGE("ParseIsoAuthClientBeginMsg failed, res: %" LOG_PUB "d.", res);
        return res;
    }
    ...
    if (res != HC_SUCCESS) {
        LOGE("PackIsoAuthServerGetTokenMsg failed, res: %" LOG_PUB "d.", res);
        return res;
    }
...
static int32_t IsoAuthServerCalTokenAndSessionKey(TaskBase *task, const CJson *in, CJson *out, int32_t *status)
{
    ...
    if (res != HC_SUCCESS) {
        LOGE("PackCalTokenAndSessionKeyMsg failed, res: %" LOG_PUB "d.", res);
        return res;
    }
...
static int32_t ProcessServerTask(TaskBase *task, const CJson *in, CJson *out, int32_t *status)
{
    ...
    if (res != HC_SUCCESS) {
        LOGE("Process iso auth server failed, step: %" LOG_PUB "d, res: %" LOG_PUB "d.", authStep, res);
    }
    return res;
}
