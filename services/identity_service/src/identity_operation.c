...
static void ReloadCredIfNeeded(int32_t osAccountId, const CJson *reqJson, const Credential *credential)
{
    ...
    if (GetBoolFromJson(reqJson, FIELD_IS_QUERY_OPEN_CRED, &isQueryOpenCred) != IS_SUCCESS) {
    ...
    if (GetBoolFromJson(extendInfoJson, FIELD_IS_OPEN_CRED_REGISTERED, &isOpenCredRegistered) != IS_SUCCESS) {
